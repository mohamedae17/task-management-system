...
- [x] **Turn 4** — Angular 21 frontend (standalone components, lazy routes, Material 21 + NgRx 21 + SignalR 10 + ng2-charts/Chart.js): Auth flow (login/register/forgot/reset), responsive shell with sidenav + theme toggle + notifications bell, Dashboard with stat cards + doughnut/bar charts + productivity table, Tasks list with filters/sort/pagination + Kanban board with CDK drag-drop wired to live `task-changed` SignalR events, Task detail with comments + assignee/label/due-date editing, Notifications page driven by NgRx + live updates, Profile with change-password tab, Admin Users management with role/active toggles. JWT interceptor with refresh-token rotation guard + error interceptor with ProblemDetails parsing.
